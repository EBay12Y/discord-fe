...
export default function ChatItem({ data }: ChatItemProps) {
  ...
  const isMe = user?.user?.id === (dm?.senderId ?? channelMsg?.member?.userId);
  ...
  const onEditFormSubmit = useCallback(
    async (values: z.infer<typeof chatInputValidator>) => {
      setIsEditing(false);
      const url = dm ? `/messages/${data?.id}` : `/messages/${data?.id}`;
      ...
      return (
        <p
          className={cn(
            ...
            deletedAt && "italic text-zinc-500 dark:text-zinc-400 text-xs mt-1"
          )}
        >
    ...
    <div className="relative group flex items-center hover:bg-black/5 p-0 md:p-4 transition w-full">
      <div className="group flex gap-x-4 w-full">
        <div className="cursor-pointer hover:drop-shadow-md transition">
          <UserAvatar
            src={
              dm?.sender?.profileUrl ?? channelMsg?.member?.user?.profileUrl!
            }
          />
        </div>
          ...
          <div className="flex items-center gap-x-2">
            <div className="flex items-center">
              <p className="font-semibold text-sm hover:underline cursor-pointer">
                {dm?.sender?.name ?? channelMsg?.member?.user?.name!}
              </p>
              {isMe && (
                ...
                <Trash
                  ...
                  onClick={() => {
                    onOpen("deleteMessage", {
                      msgUrl: dm ? `/dm/${data?.id}` : `/messages/${data?.id}`,
                    });
                  }}
                />
              </ActionTooltip>
